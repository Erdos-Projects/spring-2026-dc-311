...
def get_hourly_weather(
    ...
    # Convert UTC epoch → proper America/New_York timestamps (preserves DST transitions).
    # Do NOT add UtcOffsetSeconds() — that arithmetic trick produces correctly-valued
    # clock times but mislabels them as UTC.
    df = pd.DataFrame(
        {
            "date": pd.date_range(
...
def write_query_config(
    ...
    *config_name*.
    ...
    config = {
        ...
        "notes": (
            "Generated by modeling.data.weather_fetch.write_query_config(). "
            "Run modeling.data.weather_fetch.fetch_and_save(config_path) to download data."
        ),
    }
